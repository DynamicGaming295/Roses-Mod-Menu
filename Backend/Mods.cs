...
namespace Uptime.Backend
{
    internal class Mods : MonoBehaviour
    {
        ...
        {
            GameObject.Find("motdtext").GetComponent<Text>().text = "<color=white> Hey! This Menu was created by @dynamicgamingofficial295 for [rosevr] there are a LOT of mods on this menu and please go join the Discord" +
            ...
            GameObject.Find("WallScreenForest").GetComponent<Text>().text = "<color=yellow> Rose's Mod Menu has been successfully loaded, and is ready to be used. Click Y on left contoller to get" +
                "started.</color>";
            GameObject.Find("WallScreenCave").GetComponent<Text>().text = "<color=yellow> Rose's Mod Menu has been successfully loaded, and is ready to be used. Click Y on left contoller to get started" +
        ...
        public static void offantiban()
        {
            if (e)
            {
                e = false;
                PhotonNetwork.Disconnect();
                NotifiLib.SendNotification("<color=red>[DYNAMIC MODS]</color> Disconnected you due to used antiban in the server before, this is to evade bans!");
        ...
        {
            if (!e || Time.time < AntibanDelay + 0.6f)
            {
                if (PhotonNetwork.InRoom)
                {
                    if (Time.time > AntibanJoinDelay + 7f)
                    {
                        ...
                        if (!PhotonNetwork.CurrentRoom.CustomProperties.ToString().Contains("MODDED") || Time.time < AntibanDelay + 0.7f)
                        {
                            if (AntibanDoneBool)
                            {
                                ...
                                if (!AntibanNotifBool)
                                {
                                    NotifiLib.SendNotification("<color=red>[DYNAMIC MODS]</color> Turned on antiban!");
        ...
        public static void Save()
        {
            ...
            System.IO.Directory.CreateDirectory("DynamicPrefs");
            System.IO.File.WriteAllLines("DynamicPrefs\\dynamicSavedPrefs.txt", list);
        }

        public static void Load()
        {
            String[] thing = System.IO.File.ReadAllLines("DynamicPrefs\\DynamicSavedPrefs.txt");
